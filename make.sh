...
case "${MODE}" in
  default)
    ...
    SERIAL_TEST_REGEX="HTTP /health returns ok with valid token|HTTP /health reports db_ok false when DB is closed|HTTP docs and openapi are served without auth|Listener serves card nudge and ai status routes without regression|Listener worker-owned DB handles support concurrent mixed request load"
