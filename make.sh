...
case "${MODE}" in
  default)
    ...
    SERIAL_TEST_REGEX="HTTP /health returns ok with valid token|HTTP /health reports db_ok false when DB is closed|HTTP ai_catalog.yaml is served without auth|HTTP ai_catalog.json is served without auth|HTTP ai_catalog.yaml is served without auth \\(cloud test alias\\)|HTTP git_providers.yaml is served without auth|HTTP git_providers.json is served without auth|HTTP docs and openapi are served without auth|Listener serves card nudge and ai status routes without regression|Listener worker-owned DB handles support concurrent mixed request load"
