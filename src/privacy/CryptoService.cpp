...
std::vector<unsigned char> b64_decode(const std::string& text) {
  ...
  if (sodium_base642bin(out.data(),
    ...
    throw holder::privacy::PrivacyError(holder::privacy::PrivacyErrorCode::EnvelopeInvalid,
...
EnvelopeParts parse_envelope(const std::string& envelope) {
  ...
  if (!meta.contains("version") || meta.at("version").get<int>() != static_cast<int>(kEnvelopeVersion)) {
    throw holder::privacy::PrivacyError(holder::privacy::PrivacyErrorCode::EnvelopeInvalid,
...
std::string decrypt_envelope_v1(const std::string& envelope,
  ...
  if (!expected_key_id.empty() && parts.key_id != expected_key_id) {
    throw holder::privacy::PrivacyError(holder::privacy::PrivacyErrorCode::EnvelopeMetadataMismatch,
