...
nlohmann::json config_payload(const std::optional<holder::model::AiLocalModelConfig>& cfg) {
  return {
      {"fast_model",
       cfg.has_value() && cfg->fast_model.has_value() ? nlohmann::json(cfg->fast_model.value())
                                                      : nlohmann::json(nullptr)},
      {"strong_model",
       cfg.has_value() && cfg->strong_model.has_value() ? nlohmann::json(cfg->strong_model.value())
                                                        : nlohmann::json(nullptr)},
      {"deep_model",
       cfg.has_value() && cfg->deep_model.has_value() ? nlohmann::json(cfg->deep_model.value())
                                                      : nlohmann::json(nullptr)},
...
bool handle_ai_local_model_config_routes(
  ...
  if (path == "/ai/local-models/config" && req.method() == http::verb::put) {
    try {
      ...
      const long long updated_at = (body.contains("updated_at") && !body.at("updated_at").is_null())
