...
nlohmann::json git_test_remote_payload(
  ...
  payload["data"] = {
      ...
      {"remote_url",
       remote_url.has_value() ? nlohmann::json(remote_url.value()) : nlohmann::json(nullptr)},
      {"branch", branch},
      {"status", holder::git::remote_probe_status_name(status)}, // LCOV_EXCL_LINE
      {"remote_has_head", remote_has_head},
      {"error_code",
       reachable ? nlohmann::json(nullptr)
                 : nlohmann::json(holder::git::remote_probe_status_name(status))},
      {"error_message",
       error_message.has_value() && !error_message->empty() ? nlohmann::json(error_message.value())
                                                            : nlohmann::json(nullptr)},
  };
  return payload;
}

nlohmann::json git_push_payload(
    const std::string& project_id,
    const std::optional<std::string>& remote_url,
    const std::string& branch,
  ...
  payload["data"] = {
      ...
      {"local_head_commit",
       local_head_commit.has_value() && !local_head_commit->empty()
           ? nlohmann::json(local_head_commit.value())
           : nlohmann::json(nullptr)},
      {"error_code",
       ok ? nlohmann::json(nullptr) : nlohmann::json(holder::git::push_status_name(status))},
      {"error_message",
       error_message.has_value() && !error_message->empty() ? nlohmann::json(error_message.value())
                                                            : nlohmann::json(nullptr)},
      {"next_action",
...
bool handle_project_routes(
  ...
  if (path == "/recovery-token/import" && req.method() == http::verb::post) {
    try {
      ...
      if (!body.contains("pin") || body.at("pin").is_null() || !body.contains("recovery_token") ||
          body.at("recovery_token").is_null()) {
        ...
        if (refreshed.has_value()) {
          ...
          if (remote_configured) {
            try {
              ...
              sync_repo
                  .record_pull_result(metadata.project_id, pull_status, true, std::nullopt, now);
            } catch (const std::exception& ex) {
              pull_status = "failed";
              pull_error = ex.what();
              sync_repo
                  .record_pull_result(metadata.project_id, pull_status, false, pull_error, now);
            }
          }
          try {
            const auto metrics =
      ...
      res = support::json_response(
          project_created ? http::status::created : http::status::ok,
          payload
      );
  ...
  if (path == "/projects" && req.method() == http::verb::get) {
    try {
      ...
      std::stable_sort(
          projects.begin(),
          projects.end(),
          [order_asc, order_key](const holder::model::Project& a, const holder::model::Project& b) {
            switch (order_key) {
            case OrderKey::CreatedAt:
              return order_asc ? (a.created_at < b.created_at) : (a.created_at > b.created_at);
            case OrderKey::Name:
              return order_asc ? (a.name < b.name) : (a.name > b.name);
            case OrderKey::UpdatedAt:
            default:
              return order_asc ? (a.updated_at < b.updated_at) : (a.updated_at > b.updated_at);
            }
          }
      );
      ...
      if (offset > 0 || limit < static_cast<int>(projects.size())) {
        ...
        if (start < projects.size()) {
          ...
          paged.assign(
              projects.begin() + static_cast<std::ptrdiff_t>(start),
      ...
      for (const auto& project : projects) {
        nlohmann::json item = {
            ...
            {"git_remote_url",
             project.git_remote_url.has_value() ? nlohmann::json(project.git_remote_url.value())
                                                : nlohmann::json(nullptr)},
            {"git_provider",
             project.git_provider.has_value() ? nlohmann::json(project.git_provider.value())
                                              : nlohmann::json(nullptr)},
            {"privacy_mode", project.privacy_mode},
            {"project_key_id",
             project.project_key_id.has_value() ? nlohmann::json(project.project_key_id.value())
                                                : nlohmann::json(nullptr)},
  ...
  if (path.rfind("/projects/", 0) == 0) {
    ...
    const std::string project_id = slash_pos == std::string::npos ? suffix
                                                                  : suffix.substr(0, slash_pos);
    ...
    if (subpath == "/git/test-remote" && req.method() == http::verb::post) {
      try {
        ...
        const auto payload = git_test_remote_payload(
            ...
            probe.error_message.empty() ? std::optional<std::string>()
      ...
      try {
        ...
        if (!project.git_remote_url.has_value() || project.git_remote_url->empty()) {
          ...
          const auto payload = git_push_payload(
              project_id,
        ...
        const auto payload = git_push_payload(
            ...
            push.ahead_count,
            push.behind_count,
            push.local_head_commit.empty() ? std::optional<std::string>()
      ...
      try {
        ...
        if (!body.contains("updated_at")) {
          ...
          const bool has_privacy_mode = body.contains("privacy_mode") &&
                                        !body.at("privacy_mode").is_null();
          ...
          if (!has_name && !has_root && !has_git_remote && !has_git_provider && !has_privacy_mode &&
            ...
            if (!project_opt.has_value()) {
              ...
              const std::optional<std::string> effective_project_key_id =
                  has_project_key_id ? (body.at("project_key_id").is_null()
                                            ? std::optional<std::string>{}
                                            : std::optional<std::string>(
