...
PushResult ExecutorGitOps::push_branch(
  ...
  return executor_.call([&]() {
    return inner_.push_branch(name, branch, set_upstream);
  });
}

std::filesystem::path ExecutorGitOps::repo_dir() const {
  return executor_.call([&]() {
    return inner_.repo_dir();
  });
}
