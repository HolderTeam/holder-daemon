...
TEST_CASE("HTTP ai local model config get/put", "[http]") {
  ...
  const auto initial = http_json_request(
      bound.bind,
      bound.port,
      token,
  ...
  const auto set_all = http_json_request(
      bound.bind,
      bound.port,
      token,
  ...
  const auto clear_all = http_json_request(
      bound.bind,
      bound.port,
      token,
      boost::beast::http::verb::put,
      "/ai/local-models/config",
      nlohmann::json{{"fast_model", nullptr}, {"strong_model", nullptr}, {"deep_model", nullptr}},
      boost::beast::http::status::ok
  );
  ...
  const auto empty_string_clears = http_json_request(
      bound.bind,
      bound.port,
      token,
      boost::beast::http::verb::put,
      "/ai/local-models/config",
      nlohmann::json{{"fast_model", ""}, {"strong_model", "qwen-strong"}},
      boost::beast::http::status::ok
  );
...
TEST_CASE("HTTP ai local model config validates input and handles error branches", "[http]") {
  ...
  auto raw_bad_json = holder::test::http_request_raw(
      bound.bind,
      bound.port,
      token,
      boost::beast::http::verb::put,
  ...
  auto get_fail = http_json_request(
      bound.bind,
      bound.port,
      token,
      boost::beast::http::verb::get,
