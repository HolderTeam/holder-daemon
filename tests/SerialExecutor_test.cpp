...
TEST_CASE("SerialExecutor stop wakes blocked submit and rejects new work", "[core][executor]") {
  holder::core::SerialExecutor executor("test-executor", 1);
  ...
  REQUIRE(stop_future.wait_for(std::chrono::milliseconds(500)) == std::future_status::ready);
  REQUIRE(second_started.load());
  REQUIRE_FALSE(third_ran.load());
}
