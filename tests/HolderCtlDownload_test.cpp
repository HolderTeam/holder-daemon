...
class DownloadServer {
 public:
  explicit DownloadServer(std::string response, std::function<void()> before_response = {}) : acceptor_(context_) {
    ...
    thread_ = std::thread([this, response = std::move(response), before_response = std::move(before_response)] {
      tcp::socket socket(context_);
      boost::system::error_code error;
      while (!stopping_) {
        acceptor_.accept(socket, error);
        if (!error) break;
        if (error != boost::asio::error::would_block && error != boost::asio::error::try_again) return;
...
std::string response_bytes(const std::string& bytes, bool chunked = false) {
  ...
  if (!chunked) return response + "Content-Length: " + std::to_string(bytes.size()) + "\r\n\r\n" + bytes;
...
TEST_CASE("holderctl downloads preserve binary bytes metadata authentication and chunking", "[holderctl][download]") {
  std::string bytes;
  SECTION("empty") {}
  ...
  SECTION("larger than Beast's default buffered body limit") {
    ...
    for (std::size_t i = 0; i < bytes.size(); ++i) bytes[i] = static_cast<char>(i % 256);
  }
  for (bool chunked : {false, true}) {
    DownloadServer server(response_bytes(bytes, chunked));
    std::string downloaded;
    std::size_t largest_chunk = 0;
    const auto metadata = holder::cli::http_download(server.connection, "/asset/content",
        std::chrono::seconds(10), [&](const char* buffer, std::size_t count) {
...
TEST_CASE("holderctl downloads keep structured API errors out of binary output", "[holderctl][download]") {
  const std::string body = R"({"ok":false,"error":{"code":"storage_unavailable","message":"Storage offline","details":{"location_id":"location-1"}}})";
  DownloadServer server("HTTP/1.1 503 Service Unavailable\r\nContent-Type: application/json\r\nContent-Length: " +
      std::to_string(body.size()) + "\r\n\r\n" + body);
  bool sink_called = false;
  try {
    holder::cli::http_download(server.connection, "/asset/content", std::chrono::seconds(10),
        [&](const char*, std::size_t) { sink_called = true; });
...
TEST_CASE("holderctl downloads do not follow redirects or emit non-JSON server errors", "[holderctl][download]") {
  for (const auto& status : {"302 Found", "401 Unauthorized", "500 Internal Server Error"}) {
    DownloadServer server(std::string("HTTP/1.1 ") + status +
        "\r\nContent-Length: 5\r\nLocation: http://untrusted.invalid/\r\n\r\nerror");
    bool sink_called = false;
    REQUIRE_THROWS_AS(holder::cli::http_download(server.connection, "/asset/content", std::chrono::seconds(10),
        [&](const char*, std::size_t) { sink_called = true; }), holder::cli::CliError);
    REQUIRE_FALSE(sink_called);
  }
}

TEST_CASE("holderctl file downloads replace atomically and preserve destinations on failure", "[holderctl][download]") {
  ...
  SECTION("successful replacement, then empty replacement") {
    ...
    const auto metadata = holder::cli::download_to_file(server.connection, "/asset/content", output, std::chrono::seconds(10));
    REQUIRE(read_binary(output) == bytes);
    REQUIRE(metadata.byte_size == bytes.size());
    DownloadServer empty(response_bytes(""));
    holder::cli::download_to_file(empty.connection, "/asset/content", output, std::chrono::seconds(10));
    REQUIRE(read_binary(output).empty());
  }
  SECTION("truncated successful HTTP body") {
    DownloadServer server("HTTP/1.1 200 OK\r\nContent-Length: 99\r\n\r\npartial");
    REQUIRE_THROWS_AS(holder::cli::download_to_file(server.connection, "/asset/content", output,
        std::chrono::seconds(10)), holder::cli::CliError);
  ...
  SECTION("socket deadline expires") {
    ...
    REQUIRE_THROWS_AS(holder::cli::download_to_file(server.connection, "/asset/content", output,
        std::chrono::seconds(1)), holder::cli::CliError);
    REQUIRE(read_binary(output) == "original");
  }
