...
TEST_CASE("CardRoutes returns false for non-card paths", "[card-routes]") {
  ...
  const bool handled =
      holder::api::routes::handle_card_routes("/not-cards", req, res, db, nullptr, nullptr, uuid_v4, param_get);
...
TEST_CASE("CardRoutes context errors for missing and unknown project", "[card-routes]") {
  ...
  SECTION("missing project_id") {
    ...
    const bool handled =
        holder::api::routes::handle_card_routes("/cards/context", req, res, db, nullptr, nullptr, uuid_v4, param_get);
    REQUIRE(handled);
...
TEST_CASE("CardRoutes recent view validates limit param", "[card-routes]") {
  ...
  const bool handled =
      holder::api::routes::handle_card_routes("/cards", req, res, db, nullptr, nullptr, uuid_v4, param_get);
  REQUIRE(handled);
  REQUIRE(res.result() == http::status::bad_request);
...
TEST_CASE("CardRoutes move intent edge branches", "[card-routes]") {
  ...
  auto create = [&](const std::string& card_id,
    ...
    http::response<http::string_body> res;
    const bool handled = holder::api::routes::handle_card_routes(
        "/cards", req, res, db, &card_store, &fts, uuid_v4, param_get);
    REQUIRE(handled);
  ...
  SECTION("empty card id route") {
    auto req = make_request(http::verb::post, "/cards//move", R"({"project_id":"proj-1","intent":"to_end"})");
    http::response<http::string_body> res;
    const bool handled = holder::api::routes::handle_card_routes(
        "/cards//move", req, res, db, &card_store, &fts, uuid_v4, param_get);
    REQUIRE(handled);
    REQUIRE(res.result() == http::status::not_found);
  }
  ...
  SECTION("move route method not allowed") {
    ...
    http::response<http::string_body> res;
    const bool handled = holder::api::routes::handle_card_routes(
        "/cards/11111111-1111-4111-8111-111111111111/move", req, res, db, &card_store, &fts, uuid_v4, param_get);
    REQUIRE(handled);
    REQUIRE(res.result() == http::status::method_not_allowed);
  }

  SECTION("source not found") {
    const auto payload = call(http::verb::post,
...
TEST_CASE("CardRoutes links and backlinks edge branches", "[card-routes]") {
  ...
  SECTION("links method not allowed") {
    auto [status, payload] = call(http::verb::patch,
  ...
  SECTION("links resource target success and delete-all") {
    ...
    auto [list_status, list_payload] = call(http::verb::get,
                                            "/cards/11111111-1111-4111-8111-111111111111/links",
                                            nlohmann::json::object());
    ...
    auto [del_status, del_payload] = call(http::verb::delete_,
                                          "/cards/11111111-1111-4111-8111-111111111111/links",
                                          nlohmann::json::object());
  ...
  SECTION("backlinks card not found and method guard") {
    auto [not_found_status, not_found_payload] = call(http::verb::get,
    ...
    auto [method_status, method_payload] = call(http::verb::post,
  ...
  SECTION("backlinks include_deleted toggles filtering") {
    ...
    auto [filtered_status, filtered_payload] = call(http::verb::get,
                                                    "/cards/11111111-1111-4111-8111-111111111111/backlinks",
                                                    nlohmann::json::object());
...
TEST_CASE("CardRoutes item get and patch validation branches", "[card-routes]") {
  ...
  SECTION("get card content missing") {
    auto [created_status, created_payload] = call(http::verb::post,
                                                  "/cards",
                                                  {
